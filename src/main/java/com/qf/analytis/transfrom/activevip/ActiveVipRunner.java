 ...
 * Time:上午8:05
 * Vision:1.1
 * Description:活跃会员runner
 */
public class ActiveVipRunner implements Tool {
    private static final Logger logger = Logger.getLogger(ActiveVipRunner.class);
    private Configuration conf = new Configuration();
    //主函数－－入口
    public static void main(String[] args) {
        try {
            ToolRunner.run(new Configuration(),new ActiveVipRunner(),args);
        } catch (Exception e) {
            logger.error("Active_Member TO MYSQL IS FAILED!!",e);
        }
    }
    @Override
...
//        conf.set(GlobalConstants.RUNNING_DATE,date);
        Job job = Job.getInstance(conf, "Actice_Member_to_mysql");
        job.setJarByClass(ActiveVipRunner.class);
        //设置map参数
        job.setMapperClass(ActiveVipMapper.class);
        job.setMapOutputKeyClass(StatsUserDimension.class);
        job.setMapOutputValueClass(TimeOutputValue.class);
        //设置reduce相关参数
        //设置reduce端的输出格式化类
        job.setReducerClass(ActiveVipReducer.class);
